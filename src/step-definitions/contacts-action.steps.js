...
Then ('I check that valdation for {string} field is required', async(text) => {
    if (text === 'First Name') {
        await expect(contactPage.askUsAnythingComponent.requiredFields('firstName'))
        .toHaveAttributeContaining('aria-invalid', 'true')
    }
    if (text === 'Last Name') {
        await expect(contactPage.askUsAnythingComponent.requiredFields('lastName'))
        .toHaveAttributeContaining('aria-invalid', 'true')
    }
    if (text === 'Email') {
        await expect(contactPage.askUsAnythingComponent.requiredFields('email'))
        .toHaveAttributeContaining('aria-invalid', 'true')
    }
    if (text === 'Phone') {
        await expect(contactPage.askUsAnythingComponent.requiredFields('phone'))
        .toHaveAttributeContaining('aria-invalid', 'true')
    }
    if (text === 'How Hear About') {
        await expect(contactPage.askUsAnythingComponent.requiredFields('howHearAbout'))
        .toHaveAttributeContaining('aria-invalid', 'true')
    }
    if (text === 'GDPR Consent') {
        await expect(contactPage.askUsAnythingComponent.requiredFields('gdprConsent'))
        .toHaveAttributeContaining('aria-invalid', 'true')
    }
})
